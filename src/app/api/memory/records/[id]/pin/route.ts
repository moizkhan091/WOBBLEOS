...
const schema = z.object({ pinned: z.boolean().default(true), importance: z.number().int().min(0).max(10).optional(), actor: z.string().trim().min(1) });
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    await pinMemory({ id, pinned: parsed.data.pinned, importance: parsed.data.importance, actor: parsed.data.actor });
