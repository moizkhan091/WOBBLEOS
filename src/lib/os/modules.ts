...
export const MODULES: Record<string, ModuleDef> = {
  ...
  media: { id: "media", label: "Media Studio", title: "Media Studio", icon: "Clapperboard", status: "wired", api: "/api/media", tagline: "Submit a media generation job — durable, worker-driven, with bounded retries + crash recovery + budget caps. OpenRouter is the PRIMARY provider (images run on the same OPENROUTER_API_KEY the OS already uses — no extra key needed); fal is an OPTIONAL adapter for video/audio/3d, blocked without FAL_KEY and never auto-selected. An unconfigured provider is truthfully 'blocked', never faked." },
