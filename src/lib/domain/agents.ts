...
export const DEFAULT_AGENTS: RegisterAgentInput[] = [
  ...
  // Qualification Council — 8 governed roles (the WOBBLE qualification filters). Each blends a deterministic
  // policy signal from CRM data with an evidence-LLM score; the council rolls up an A-E grade.
  { slug: "qual_real_problem_agent", name: "Real-Problem Council Member", role: "qualifier", module: "qualification_council", team: "sales", purpose: "Judges whether the prospect has a real, describable business problem AI can attack (not curiosity).", inputTypes: ["crm_company"], outputTypes: ["qualification_score"], tools: ["openrouter"], memoryBanks: ["company"], costProfile: "mid" },
  { slug: "qual_budget_agent", name: "Budget Council Member", role: "qualifier", module: "qualification_council", team: "sales", purpose: "Judges real budget for a paid audit + implementation, using company size/stage signals (deterministic policy + LLM).", inputTypes: ["crm_company"], outputTypes: ["qualification_score"], tools: ["openrouter"], memoryBanks: ["company"], costProfile: "mid" },
  { slug: "qual_urgency_agent", name: "Urgency Council Member", role: "qualifier", module: "qualification_council", team: "sales", purpose: "Judges owner seriousness and urgency vs mere exploration.", inputTypes: ["crm_company"], outputTypes: ["qualification_score"], tools: ["openrouter"], memoryBanks: ["company"], costProfile: "mid" },
  { slug: "qual_access_agent", name: "Access Council Member", role: "qualifier", module: "qualification_council", team: "sales", purpose: "Judges whether WOBBLE can access enough workflows/people/tools/data to audit and build (policy + LLM).", inputTypes: ["crm_company"], outputTypes: ["qualification_score"], tools: ["openrouter"], memoryBanks: ["company"], costProfile: "mid" },
  { slug: "qual_learn_agent", name: "Willingness-to-Learn Council Member", role: "qualifier", module: "qualification_council", team: "sales", purpose: "Judges willingness to adopt/learn AI rather than outsource to an agency.", inputTypes: ["crm_company"], outputTypes: ["qualification_score"], tools: ["openrouter"], memoryBanks: ["company"], costProfile: "mid" },
  { slug: "qual_phased_agent", name: "Phased-Implementation Council Member", role: "qualifier", module: "qualification_council", team: "sales", purpose: "Judges willingness to implement in phases rather than demanding everything at once.", inputTypes: ["crm_company"], outputTypes: ["qualification_score"], tools: ["openrouter"], memoryBanks: ["company"], costProfile: "mid" },
  { slug: "qual_workflow_agent", name: "High-Value-Workflow Council Member", role: "qualifier", module: "qualification_council", team: "sales", purpose: "Judges whether there is a clear high-value first workflow (a strong wedge) to start with.", inputTypes: ["crm_company"], outputTypes: ["qualification_score"], tools: ["openrouter"], memoryBanks: ["company", "offer"], costProfile: "mid" },
  { slug: "qual_complexity_agent", name: "Operational-Complexity Council Member", role: "qualifier", module: "qualification_council", team: "sales", purpose: "Judges whether there is enough operational complexity for AI to matter, using industry signals (policy + LLM).", inputTypes: ["crm_company"], outputTypes: ["qualification_score"], tools: ["openrouter"], memoryBanks: ["company"], costProfile: "mid" },
  // Discovery & Meeting Intelligence — extracts typed discovery facts (pain/budget/authority/need/timeline/
  // stack/objection/next-step/risk) from a meeting transcript, each with confidence + a verbatim snippet, for founder review.
  // Revenue/CRM additions. These were declared ahead of their execution path and stayed `paused` until
  // the code that runs them existed: the registry-integrity guard refuses to let an agent claim `active`
  // with nothing behind it, and a roster of decorative agents is worse than a short one. All four of the
  // deal team now run through src/lib/deal-team.
  // Revenue/CRM additions. The question engine was shipped unregistered, so it ran without appearing in
  // Agent Registry or being governed like its 25 peers; the rest are the teammates the department was
  // missing between "we have a call booked" and "they signed".
  { slug: "call_question_engine", name: "Pre-Call Question Engine", role: "strategist", module: "call_questions", team: "sales", purpose: "Writes the first-call questions for ONE client from their form answers, approved findings and the WOBBLE service menu; never recycles a bank, and enforces a coverage spine so nothing needed to price an audit is forgotten.", inputTypes: ["client_context"], outputTypes: ["question_set"], tools: ["openrouter"], memoryBanks: ["company", "offer"], modelRole: "call_questions", costProfile: "strong" },
  { slug: "revenue_head", name: "Head of Revenue", role: "orchestrator", module: "crm", team: "commercial", purpose: "Conversational head of Revenue and CRM: reads any client's full container and drives the department's deterministic services and revision cycles on the founder's instruction. Prepares, never sends, never moves money.", inputTypes: ["instruction"], outputTypes: ["action", "revision", "answer"], tools: ["openrouter", "jobs"], memoryBanks: ["company", "offer", "founder_taste"], modelRole: "revenue_head", costProfile: "strong" },
  { slug: "revision_specialist", name: "Revision Specialist", role: "consultant", module: "proposals", team: "commercial", purpose: "Turns a founder's change request into a scoped revision of an artifact: decides which components must be reran, reruns only those, and reports the diff for approval.", inputTypes: ["revision_request"], outputTypes: ["artifact_version"], tools: ["openrouter"], memoryBanks: ["company", "offer"], costProfile: "strong" },
  { slug: "deal_reviewer", name: "Deal Reviewer", role: "qa", module: "proposals", team: "commercial", purpose: "Argues the client's side before a proposal is sent: what would make THIS client say no, what is unproven, what is priced wrong. Adversarial by design, never congratulatory.", inputTypes: ["proposal"], outputTypes: ["critique"], tools: ["openrouter"], memoryBanks: ["company", "offer"], modelRole: "deal_review", costProfile: "strong" },
  { slug: "objection_handler", name: "Objection Handler", role: "consultant", module: "crm", team: "commercial", purpose: "Turns approved call findings into the objections this client will actually raise and the answer to each, in their own language.", inputTypes: ["discovery_fact"], outputTypes: ["objection_brief"], tools: ["openrouter"], memoryBanks: ["company", "offer"], modelRole: "objection_handling", costProfile: "mid" },
  { slug: "pricing_analyst", name: "Pricing Analyst", role: "analyst", module: "proposals", team: "commercial", purpose: "Sanity-checks a quote against WOBBLE's own pricing history and the client's stated economics, so two similar clients cannot be quoted wildly different numbers. Advisory only; a founder sets every price.", inputTypes: ["proposal"], outputTypes: ["pricing_opinion"], tools: ["openrouter"], memoryBanks: ["offer", "company"], modelRole: "pricing_analysis", costProfile: "mid" },
  { slug: "follow_up_writer", name: "Follow-up Writer", role: "copywriter", module: "crm", team: "commercial", purpose: "Drafts the actual next message to a client in the right channel and their own register, grounded in what was said on the call. Drafts only; a founder sends.", inputTypes: ["client_context"], outputTypes: ["message_draft"], tools: ["openrouter"], memoryBanks: ["company", "brand", "founder_taste"], modelRole: "follow_up_writing", costProfile: "mid" },
  { slug: "meeting_intelligence_analyst", name: "Meeting Intelligence Analyst", role: "analyst", module: "meeting_intelligence", team: "sales", purpose: "Reads a discovery/meeting transcript and extracts typed discovery facts with confidence + source snippets, landing pending_review (nothing trusted until a founder approves).", inputTypes: ["transcript", "meeting"], outputTypes: ["discovery_fact"], tools: ["openrouter"], memoryBanks: ["company", "client"], costProfile: "mid" },
];
