...
export const DEFAULT_AGENTS: RegisterAgentInput[] = [
  ...
  // Revenue/CRM additions. The six marked `paused` are declared ahead of their execution path and
  // stay paused until the code that runs them exists: the registry-integrity guard refuses to let an
  // agent claim `active` with nothing behind it, and a roster of decorative agents is worse than a short one.
