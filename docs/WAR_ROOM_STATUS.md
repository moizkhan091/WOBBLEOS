...
- [ ] **NEXT — proposal-accept → sales_crm origination migration.** Today `proposalAction` accept still fires the inline invoice+won+delivery chain. Migrate it to a single `proposal_artifact` dispatch to `sales_crm` so the commercial DEPARTMENT chain owns won→invoice→delivery (avoids the double-invoice conflict). Requires updating `proposal-department.test.ts` (accept-fires-inline assertions). This makes the commercial departments production-triggered end-to-end.
- [ ] **NEXT — reviewer MED #5 / LOW #7.** Make `proposalAction` accept atomic/redrivable (currently 3 non-atomic writes can orphan an invoice); add a `dataClassification` gate at dispatch (defense-in-depth) in `planDepartmentRoute`.
...
- [x] **L4 — Playwright E2E framework** — committed + proven LOCALLY (10 tests, real Postgres + Chromium, 6.6m green) asserting REAL DB effects (resume→escalation resolved AND handoff redriven; retry→delivered; cancel→cancelled; budget/KPI verified usage), authenticated founder + unauth gate, isolated seed + teardown. `@playwright/test` + e2e scripts in package.json; CI Postgres `e2e` job in ci.yml. **NOT YET GREEN IN CI:** the first CI run of the e2e job failed (candidate: Secure login cookie over http in `next start` → run the E2E server with `NODE_ENV=development`; or browser-deps/startup). The job is gated to `workflow_dispatch` (manual) so it never reds push CI. **NEXT:** trigger it manually (Actions → CI → Run workflow), fix the CI-env issue, then promote to on-push + required. Phase 3's browser gate is proven locally; the CI gate is pending stabilisation.
