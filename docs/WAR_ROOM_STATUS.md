...
- [ ] **NEXT — reviewer LOW #7 (Priority 4).** Add a `dataClassification` gate at DISPATCH time (`planDepartmentRoute` / `validateHandoff`), not only at accept — defense-in-depth so an unauthorized-classification route is rejected before persistence. (The origination proof already surfaced + fixed one real gap: Finance now permits `client_confidential` for client deals.)
