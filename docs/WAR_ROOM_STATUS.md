...
- **Priority 3 — make QA boards gate LIVE workflows.** The QA framework (Phase 4, `src/lib/qa/*`) is proven + persists (`qa_reviews`) but does NOT yet gate any live flow. Wire independent QA gates into ≥5 flows: Paid Audit→Proposal, Proposal technical review, Proposal commercial review, Research validation, Content quality/brand. For each: authoring dept produces candidate → independent board (distinct evaluator identity) receives evidence → pass permits downstream / fail blocks completion / revise routes to the exact failed stage / blocked escalates. Register QA reviewer identities + memberships in the seed (`quality_assurance` dept). Prove via Postgres + Playwright. (The author identity must never approve its own output — the framework already enforces this.)
- **Priority 5 — complete escalation-control proof.** `resume`/`terminate`/`dismiss` have REAL execution semantics (proven: DB + Playwright). **GAP FOUND:** `reroute` is a LABEL only — `resolveEscalation` records `resolutionAction:"reroute"` but performs NO real alternate-route execution. Implement reroute: create a valid, authorized ALTERNATE route for the blocked work (re-dispatch the blocked handoff to a different permitted destination) without losing lineage. Then add integration + Playwright coverage for all four actions incl. duplicate clicks, invalid transitions, and unauthorized users — passing in REQUIRED CI (not only locally).
