...
- **Branch:** `main` · **HEAD:** `bbb2d75` (Phase-3 Command Centre UI) · atop the full department runtime (`66e1e22` health, `25eb23f` paid-audit vertical, `7e075ed` orchestrator, `ea50795` team→FK, `f959deb` domain model).
- **Latest migration:** `0033_department_members` (35 applied to the local dev DB; zero drift). 0030 handoff runtime · 0031 approval effects · 0032 departments · 0033 department_members. **Gate:** typecheck 0 · **656 tests (87 files)** · build 0. Real-OpenRouter smoke PASS; real-DB proofs PASS for checkpoints, payments ledger, **handoff runtime**, **approval-effects outbox**, **memory-update outbox**, **graph→handoff execution backbone**, **Command Centre handoff ops**, and the full **department runtime** (domain model, membership, registry, seed, registry-driven roll-up, orchestrator routing, Paid Audit vertical, truthful health). Command Centre UI browser-verified (inspect + operate).
...
- **Phase 3 (real departments): IN PROGRESS — building the real department runtime (not just observability).**
  ...
  - **Batch 3b — canonical org SEED (`src/lib/departments/seed.ts`, wired into `db:seed`):** 13 real departments (research_intelligence, free_audit, paid_audit, proposal, content, design_intelligence, media_production, publishing, sales_crm, finance, delivery, security_governance, founder_command_centre) with policies + products + downstream routing, and 10 operational memberships. TRUTHFUL status: paid_audit + content are `active` (real orchestrator + full specialist team); everything not-yet-built is `draft` — no shallow-active records. Idempotent upsert (safe on every boot). Real-DB proof (verify-department-seed-db.ts) — org left in the dev DB.
