...
- **HEAD:** `efb11d6` (per-department detail). **Gate:** typecheck 0 · **588 tests (79 files)** · build 0. Real-OpenRouter smoke PASS; real-DB proofs PASS for checkpoints, payments ledger, **handoff runtime**, **approval-effects outbox**, **memory-update outbox**, **graph→handoff execution backbone**, **Command Centre handoff ops**, and **department roll-up**.
...
- **Phase 3 (real departments): STARTED — department observability layer done.** `88feb70` department roll-up (per-department live handoff activity: in-flight/completed/stuck, spend, weighted quality + the registered team; `GET /api/departments`, real-DB proof) + `efb11d6` per-department detail (the agent team with run/failure/quality + recent handoffs; `GET /api/departments/[department]`). Departments are now first-class, observable units, built on the handoff `department` tag the execution backbone already emits — no new schema. **Next in Phase 3:** per-department QA/quality-gate boards (Phase 4), then the remaining program.
